"""Benchmark production CPU and CuPy Richardson--Lucy TV on large 3D stacks.
...
SCHEMA = "napari-vipp-cupy-rl-tv-performance-evidence"
SCHEMA_VERSION = 1
GENERATOR_ID = "numpy-pcg64-large-rl-tv-v1"
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="practical",
        help=(
            "practical runs the 64x512x512 synthetic shape stress; optionally "
            "add one lazily sliced real ND2 volume with --nd2."
        ),
    )
    parser.add_argument(
        "--nd2",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.validate_existing is not None:
        try:
            ...
            print(
                f"RL-TV performance evidence validation failed: {exc}",
                file=sys.stderr,
            )
            return 2
        print(f"RL-TV performance evidence is current: {artifact}")
        return 0
    ...
    try:
        ...
        print(f"RL-TV performance benchmark failed: {exc}", file=sys.stderr)
        return 2
    except Exception as exc:  # optional provider failures need a concise CLI edge
        print(
            f"RL-TV performance benchmark failed: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 2
    print(f"Wrote RL-TV performance evidence to {args.output.resolve()}")
...
def run_benchmarks(
    *,
    ...
    if selected_profile != "practical":
        raise ValueError("profile must be 'practical'.")
    ...
    try:
        ...
        definitions = list(SYNTHETIC_CASES)
        ...
        for case_index, definition in enumerate(definitions, start=1):
            ...
            preflight_workload = WorkloadDescriptor(
                node_id=f"rl-tv-performance-{definition.case_id}",
            ...
            call = PreparedNodeCall(
                node_id=f"rl-tv-performance-{definition.case_id}",
                operation_id=OPERATION_ID,
                cpu_function=richardson_lucy_tv_deconvolution,
                inputs=(image, psf),
                kwargs=parameters,
...
def render_markdown(document: Mapping[str, object]) -> str:
    ...
    lines = [
        ...
        f"- TV regularization: `{float(method['tv_regularization']):g}`",
        f"- TV epsilon: `{float(method['tv_epsilon']):.0e}`",
        f"- Filter epsilon: `{float(method['filter_epsilon']):.0e}`",
        f"- Denominator floor: `{float(method['denominator_floor']):g}`",
...
def _validate_document_contract(document: Mapping[str, object]) -> None:
    ...
    if profile != "practical":
        raise PerformanceBenchmarkError("Evidence profile must be practical.")
    try:
...
def _validate_case_contract(case: Mapping[str, object]) -> None:
    ...
    if memory.get("model_id") != "cupyx-richardson-lucy-tv-fft-memory-v1":
        raise PerformanceBenchmarkError("Unexpected RL-TV memory model.")
