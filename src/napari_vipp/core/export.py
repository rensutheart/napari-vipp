...
def _build_header(pipeline: PrototypePipeline) -> str:
    ...
    return (
        ...
        "Local files opened with load_image are content-hashed and verified "
        "around the read so output provenance binds their exact source "
        "revision. The convenience folder loop does not provide durable "
        "batch resume guarantees.\n\n"
        "Run a single image:\n"
...
def _build_function_body(
    ...
    lines.extend(
        (
            ...
            f"{_INDENT}ImageDataset or SourcePayload (as returned by load_image)",
            f"{_INDENT}carries the complete normalized ImageState into scientific",
            f"{_INDENT}operations. Local paths opened with load_image are hashed",
            f"{_INDENT}before reading and verified after materialization. Raw arrays",
            f"{_INDENT}carry only source identity explicitly supplied by the caller.",
            f'{_INDENT}"""',
...
def _build_main(source_ids: list[str], function_name: str) -> str:
    ...
    lines = [
        ...
        f"{_INDENT}{_INDENT}finally:",
        f"{_INDENT}{_INDENT}{_INDENT}publication_entries = None",
        f"{_INDENT}{_INDENT}{_INDENT}output = None",
        f"{_INDENT}{_INDENT}{_INDENT}results = None",
        f"{_INDENT}{_INDENT}records.append(record)",
