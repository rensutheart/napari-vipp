...
_SIGMA_FILTER_SUPPORTED_DTYPES = frozenset(
    {np.dtype(np.uint8), np.dtype(np.uint16), np.dtype(np.float32)}
)
_SIGMA_FILTER_ROW_BLOCK_SIZE = 64
_SIGMA_FILTER_FLOAT32_SQUARE_LIMIT = float(
...
def sigma_filter_footprint(
    ...
    the same order.
    """
    normalized = _validated_sigma_filter_float(
        radius,
        name="radius",
        minimum=0.5,
        maximum=10.0,
    ...
    if 1.5 <= normalized < 1.75:
        ...
        normalized = 2.85
    return _cached_sigma_filter_footprint(normalized)


@lru_cache(maxsize=128)
def _cached_sigma_filter_footprint(
...
def sigma_filter(
    ...
    channel_axis = _validated_filter_channel_axis(
        channel_axis,
        arr.ndim,
        operation="Sigma Filter",
    )
    if arr.dtype not in _SIGMA_FILTER_SUPPORTED_DTYPES:
        raise ValueError(
    ...
    sigma_width = _validated_sigma_filter_float(
        sigma_width,
        name="sigma_width",
        minimum=0.0,
    )
    minimum_pixel_fraction = _validated_sigma_filter_float(
    ...
    row_blocks = math.ceil(rows / _SIGMA_FILTER_ROW_BLOCK_SIZE)
    ...
    completed_blocks = _validate_sigma_filter_working_input(
        working,
        progress=progress,
    ...
    for plane_index in plane_indices:
        ...
        for row_start in range(0, rows, _SIGMA_FILTER_ROW_BLOCK_SIZE):
            if progress is not None:
                progress.check_cancelled()
            row_stop = min(row_start + _SIGMA_FILTER_ROW_BLOCK_SIZE, rows)
            filtered = _sigma_filter_row_block(
                source_plane,
                padded,
                row_start=row_start,
                row_stop=row_stop,
            ...
            destination_plane[row_start:row_stop] = _restore_sigma_filter_dtype(
                filtered,
                arr.dtype,
            )
            completed_blocks += 1
...
def _validate_sigma_filter_working_input(
    working: np.ndarray,
    *,
    progress,
    total_blocks: int,
    ...
    for plane_index in plane_indices:
        ...
        for row_start in range(0, plane.shape[0], _SIGMA_FILTER_ROW_BLOCK_SIZE):
            if progress is not None:
                progress.check_cancelled()
            block = np.asarray(
                plane[row_start : row_start + _SIGMA_FILTER_ROW_BLOCK_SIZE],
                dtype=np.float32,
            )
            ...
            if working.dtype == np.float32 and float(np.max(np.abs(block))) > (
                _SIGMA_FILTER_FLOAT32_SQUARE_LIMIT
            ):
                raise ValueError(
...
def _validated_sigma_filter_float(
    value,
    *,
    name: str,
    minimum: float,
...
def _restore_sigma_filter_dtype(
    values: np.ndarray,
    dtype: np.dtype,
) -> np.ndarray:
