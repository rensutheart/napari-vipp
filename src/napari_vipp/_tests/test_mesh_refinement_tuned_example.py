...
def _example():
    spec = _example_workflow_by_id("mesh-refinement-tuned")
...
def test_tuned_example_preserves_authored_parameters_and_packaged_copy():
...
def test_tuned_mesh_example_runs_with_five_calibrated_objects_without_writing(
...
def test_tuned_example_can_be_selected_and_loaded_in_widget(qtbot):
    ...
    dialog.filter_edit.setText("Tuned Refinement")
    dialog.select_example("mesh-refinement-tuned")
    assert dialog.selected_example().title == "Mesh Objects — Tuned Refinement"
    assert dialog.open_button.isEnabled()
    widget = VippWidget(_Viewer())
    qtbot.addWidget(widget)
    path = widget.load_example_workflow("mesh-refinement-tuned")
    assert path.name == "synthetic-mesh-refinement-tuned.json"
