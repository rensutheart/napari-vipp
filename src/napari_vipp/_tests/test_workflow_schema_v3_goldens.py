...
def _canonical_schema_v6_document(document: dict[str, Any]) -> dict[str, Any]:
    ...
    for node in canonical["nodes"]:
        if node["operation_id"] == "binary_threshold":
            node["params"].setdefault("foreground", "Above")
            node["params"].setdefault("low_threshold", 0.25)
...
def test_showcase_a2_additions_preserve_the_prior_nine_lane_analysis():
    document = _without_showcase_a2_additions()
