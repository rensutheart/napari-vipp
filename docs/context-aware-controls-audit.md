...
| Crop Stack | `channel_axis` override | Explicit channel semantics and authored override | Saved non-negative override: show. One explicit channel axis or explicit scalar input with automatic `-1`: hide. Missing, unresolved, or ambiguous semantics: show. | Crop never removes channel samples. The axis is protected while resolving Y/X; metadata supplies it automatically when unambiguous, while a saved manual choice remains reviewable. | Implemented: `crop_channel_override`. |
...
| Clamp Intensity | `minimum`, `maximum` | `cutoff_mode` | Values: show. Data range: hide. Missing mode: show. | Explicit cutoffs are unused in data-range mode. | Implemented: `parameter_in`. |
| Seven colocalization families, including Object Colocalization | channel thresholds | `threshold_mode` | Always show. Manual: editable. Costes auto: read-only. Unknown: editable. | Costes derives both thresholds, but the resolved values remain useful scientific output and become the starting point when the user switches to Manual or drags a scatter guide. | Implemented across all 14 threshold declarations. |
