...
| Format family | Reader route | Notes |
| --- | --- | --- |
| Nikon ND2 | Optional `nd2` reader first | VIPP 0.14.0a1 has lazy inspection/data, decoded-size estimates, stable item identity, and inspection/read calibration, channel, objective, and acquisition parity on the frozen sources. |
| Zeiss CZI / LSM | Optional `czifile` for CZI, TIFF/LSM path for LSM, BioIO fallback | VIPP 0.14.0a1 preserves stable CZI scene keys/pixels and LSM main-versus-RGB-thumbnail identity with metadata parity. Both native pixel routes remain eager and no CZI pyramid claim is made. |
| Leica LIF / LOF / XLIF | Optional `liffile` first, BioIO/Bio-Formats fallback | VIPP 0.14.0a1 preserves LIF calibration, channels, and objective metadata. Native access is eager; VIPP pins/refuses reader topology changes because PR2729 is one combined item in `liffile` and four in Bio-Formats. LOF/XLIF remain unqualified without frozen sources. |
| Olympus OIR / OIB / OIF / VSI | Optional `oirfile`/`oiffile` where available, BioIO/Bio-Formats fallback | VIPP 0.14.0a1 preserves OIR TCYX and OIB CZYX metadata contracts, binds OIF and VSI/ETS companion trees into source revision, and reports Java/Bio-Formats readiness actionably. Native OIR/OIB reads remain eager; VSI full decode is cache-gated. |
| Imaris IMS | BioIO/Bio-Formats fallback | VIPP 0.14.0a1 preserves logical TCZYX dimensions and bounded metadata with decoded-size reporting. Java/codecs remain optional prerequisites and pyramid enumeration/cheap level reads are not claimed. |
