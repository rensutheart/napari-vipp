...
| OME-Zarr preview and lazy execution strategy | Candidate core delivered | Local OME-Zarr 0.4/0.5 preview selects and slices a declared lower level, preserves labels, supports cancellation/stale generations, reports observable I/O, and visibly keeps level-0 analysis. | Qualify the integrated UI journey; later add remote stores, richer controls, operation capability declarations, and graph-level lazy/chunked execution. |
...
| --- | --- | --- |
| Nikon ND2 | Optional `nd2` reader first | The 0.14.0a1 candidate has lazy inspection/data, decoded-size estimates, stable item identity, and inspection/read calibration, channel, objective, and acquisition parity on the frozen sources. |
| Zeiss CZI / LSM | Optional `czifile` for CZI, TIFF/LSM path for LSM, BioIO fallback | The candidate preserves stable CZI scene keys/pixels and LSM main-versus-RGB-thumbnail identity with metadata parity. Both native pixel routes remain eager and no CZI pyramid claim is made. |
| Leica LIF / LOF / XLIF | Optional `liffile` first, BioIO/Bio-Formats fallback | The candidate preserves LIF calibration, channels, and objective metadata. Native access is eager; VIPP pins/refuses reader topology changes because PR2729 is one combined item in `liffile` and four in Bio-Formats. LOF/XLIF remain unqualified without frozen sources. |
| Olympus OIR / OIB / OIF / VSI | Optional `oirfile`/`oiffile` where available, BioIO/Bio-Formats fallback | The candidate preserves OIR TCYX and OIB CZYX metadata contracts, binds OIF and VSI/ETS companion trees into source revision, and reports Java/Bio-Formats readiness actionably. Native OIR/OIB reads remain eager; VSI full decode is cache-gated. |
| Imaris IMS | BioIO/Bio-Formats fallback | The candidate preserves logical TCZYX dimensions and bounded metadata with decoded-size reporting. Java/codecs remain optional prerequisites and pyramid enumeration/cheap level reads are not claimed. |
