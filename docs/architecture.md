...
src/napari_vipp/
  ...
  ui/
    ...
    napari_compat.py   feature-detected camera and native-window access
...
| Source presentation preview | `core/source_preview.py`, `ui/source_preview.py` | A lower local OME-Zarr level may be read within a strict decoded-byte budget for early presentation only. Volumetric previews can retain the safe level's Z extent while T/C remain selected. Generation checks and cancellation prevent stale publication. Preview bytes never replace SourceItem level-0 analysis, scientific cache data, or provenance. |
...
| napari/Qt presentation compatibility | `ui/napari_compat.py`, `_widget.py`, `scripts/capture_vipp_docs.py` | Public behavior is selected by feature detection, not napari or binding version tests. Camera access prefers `viewer.scene.camera` and falls back to the older `viewer.camera`. Native-window lookup first traverses Qt parents from an owned widget or napari's exposed Qt viewer and keeps one bounded `_qt_window` fallback for older supported napari. The same paths run through `qtpy` under PyQt6 on Windows/Linux and PySide6 on macOS. |
...
| Viewer presentation | `_widget.py`, `core/diagnostics.py` | Generated inspect and pin layers receive non-writeable views of cached arrays, preventing napari writes without duplicating full volumes; a Labels layer alone may require a presentation conversion. Compatible Image layers reuse the same layer object while their data reference and display properties are reset. Generated Image/Labels layers also receive carried axis names when the installed napari API supports them; labels align to displayed dimensionality, exclude a hidden RGB/RGBA component axis, and follow the selected scientific layer rather than a hidden/provisional source preview. These labels are display state and never feed back into `ImageState`. Large generated layers and incremental node cards may start with provisional dtype-based contrast solely to keep Qt responsive, followed by exact finite extrema in a stale-safe worker after final graph publication. Viewer contrast, labels, colormaps, and thumbnails never enter operation inputs. |
