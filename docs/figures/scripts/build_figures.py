...
class SVG:
    ...
    def text(
        ...
        *,
        ...
        self.parts.append(
            ...
            f'text-anchor="{_escape(anchor)}"{fill_attr}>' + "".join(tspans) + "</text>"
        )
    ...
    def render(self) -> str:
        markers = """
<defs>
  <marker id="arrow" viewBox="0 0 10 10" refX="8.8" refY="5"
    markerWidth="10" markerHeight="10" orient="auto"
    markerUnits="userSpaceOnUse">
    <path d="M0 0 L10 5 L0 10 Z" fill="#3e4b63"/>
  </marker>
  <marker id="arrow-blue" viewBox="0 0 10 10" refX="8.8" refY="5"
    markerWidth="10" markerHeight="10" orient="auto"
    markerUnits="userSpaceOnUse">
    <path d="M0 0 L10 5 L0 10 Z" fill="#117ea6"/>
  </marker>
  <marker id="arrow-violet" viewBox="0 0 10 10" refX="8.8" refY="5"
    markerWidth="10" markerHeight="10" orient="auto"
    markerUnits="userSpaceOnUse">
    <path d="M0 0 L10 5 L0 10 Z" fill="#7251b5"/>
  </marker>
  <marker id="arrow-green" viewBox="0 0 10 10" refX="8.8" refY="5"
    markerWidth="10" markerHeight="10" orient="auto"
    markerUnits="userSpaceOnUse">
    <path d="M0 0 L10 5 L0 10 Z" fill="#23816f"/>
  </marker>
  <marker id="arrow-amber" viewBox="0 0 10 10" refX="8.8" refY="5"
    markerWidth="10" markerHeight="10" orient="auto"
    markerUnits="userSpaceOnUse">
    <path d="M0 0 L10 5 L0 10 Z" fill="#a9680b"/>
  </marker>
  <marker id="arrow-red" viewBox="0 0 10 10" refX="8.8" refY="5"
    markerWidth="10" markerHeight="10" orient="auto"
    markerUnits="userSpaceOnUse">
...
def _type_chip(
    ...
    *,
    ...
    svg.rect(
        x, y, width, 28, cls="mini-card", rx=14, extra=f'fill="{fill}" stroke="{color}"'
    )
...
def _file_icon(svg: SVG, x: float, y: float, *, color: str, scale: float = 1.0) -> None:
    svg.raw(
        f'<path d="M{x:g} {y:g}h{42 * scale:g}l{17 * scale:g} '
        f'{17 * scale:g}v{58 * scale:g}h{-59 * scale:g}z" '
        f'fill="#ffffff" stroke="{color}" stroke-width="3" '
        'vector-effect="non-scaling-stroke"/>'
        f'<path d="M{x + 42 * scale:g} {y:g}v{17 * scale:g}h{17 * scale:g}" '
...
def _layer_icon(svg: SVG, x: float, y: float, *, color: str) -> None:
    for dx, dy in ((0, 0), (10, 9), (20, 18)):
        svg.rect(
            ...
            cls="mini-card",
            rx=6,
            extra=f'fill="#ffffff" stroke="{color}"',
        )


def _graph_icon(svg: SVG, x: float, y: float, *, color: str = "#7251b5") -> None:
    svg.path(
        f"M{x + 50:g} {y + 25:g}H{x + 80:g}V{y + 5:g}H{x + 105:g}",
        cls="connector",
        marker=None,
    )
    svg.path(
        f"M{x + 50:g} {y + 25:g}H{x + 80:g}V{y + 55:g}H{x + 105:g}",
        cls="connector",
        marker=None,
    )
    svg.rect(
        x, y + 5, 50, 50, cls="mini-card", rx=9, extra='fill="#ffffff" stroke="#117ea6"'
    )
    svg.rect(
        x + 105,
        y - 5,
    ...
    svg.rect(
        ...
        cls="mini-card",
        rx=9,
        extra='fill="#ffffff" stroke="#23816f"',
    )
...
def build_system_overview() -> None:
    svg = SVG(
        ...
        "Scientific inputs enter a napari workspace containing the viewer and the "
        "VIPP typed workflow editor. Workflow execution uses CPU reference "
        "kernels and eligible optional GPU segments. Accepted results become "
        "viewer layers, saved images and tables, reusable workflows, and "
        "provenance-bearing run records.",
    )

    with svg.group("title-band", label="Figure title"):
        svg.text(
            60, 62, "napari-vipp: visual workflows for bioimage analysis", cls="title"
        )
        svg.text(
            60,
            104,
            "A napari-native, typed workflow environment for interactive "
            "analysis, reusable execution, and traceable outputs.",
            cls="subtitle",
        )
    ...
    with svg.group("scientific-inputs", label="Scientific input sources"):
        ...
        svg.text(
            160,
            668,
            ["bundled samples", "optional readers"],
            cls="small",
            line_height=20,
        )
    ...
    with svg.group("napari-workspace", label="napari viewer and VIPP workflow"):
        ...
        svg.raw(
            '<path d="M414 348 C454 308 477 352 512 319 S548 326 551 300" '
            'fill="none" stroke="#d1d8e3" stroke-width="3"/>'
        )
    ...
    with svg.group("workflow-execution", label="Workflow execution"):
        ...
        svg.text(
            980,
            308,
            ["semantic axes", "physical-grid contracts"],
            cls="small",
            anchor="middle",
            line_height=24,
        )
        ...
        svg.text(
            980,
            662,
            ["eligible CUDA segments", "visible CPU fallback"],
            cls="small",
            anchor="middle",
            line_height=24,
        )
    ...
    with svg.group("results-and-reuse", label="Results and reproducibility"):
        ...
        cards = [
            (
                235,
                "Interactive layers",
                ("images · label images", "comparison overlays"),
            ),
        ...
        for index, (y, heading, lines) in enumerate(cards):
            ...
            if index == 0:
                ...
                svg.raw(
                    f'<path d="M1178 {y + 66:g}l15 -18 10 10 11 -22" fill="none" '
                    'stroke="#a9680b" stroke-width="3"/>'
                )
            elif index == 1:
                ...
                svg.rect(
                    ...
                    cls="mini-card",
                    rx=5,
                    extra='fill="#ffffff" stroke="#a9680b"',
                )
                svg.line(1187, y + 35, 1207, y + 35, cls="connector-amber")
                svg.line(1187, y + 48, 1207, y + 48, cls="connector-amber")
            else:
                svg.circle(
                    1197, y + 47, 19, fill="#ffffff", stroke="#a9680b", cls="port"
                )
                svg.raw(
                    f'<path d="M1188 {y + 47:g}l6 7 12 -16" fill="none" '
                    'stroke="#a9680b" stroke-width="3"/>'
                )
    ...
    with svg.group(
        "scientific-context-band", label="Scientific context carried through execution"
    ):
        ...
        svg.text(
            360,
            834,
            "semantic axes · scale and units · source revision · actual "
...
def build_workflow() -> None:
    svg = SVG(
        ...
        "calculates and inspects intermediate results, and iteratively refines "
        "the workflow. The reviewed graph can be reopened in VIPP, used from "
        "generated Python or a command line, or combined with a reviewed batch "
        "configuration. All routes converge on the shared scientific execution "
        "service.",
    )
    ...
    with svg.group("title-band", label="Figure title"):
        ...
        svg.text(
            60,
            104,
            "A reviewed VIPP workflow can be reopened, exported, or applied to "
            "local image collections through the same execution service.",
            cls="subtitle",
        )
    ...
    with svg.group("workflow-connectors", label="Workflow sequence and iteration"):
        ...
        svg.text(
            730,
            506,
            "revise nodes, connections, or parameters",
            cls="arrow-label",
            anchor="middle",
        )
        for start, end in ((1095, 1050), (750, 705), (405, 360)):
            svg.path(
                f"M{start - 8:g} 738 H{end + 8:g}", cls="connector", marker="arrow"
            )

    with svg.group(
        "interactive-authoring-steps", label="Four interactive authoring steps"
    ):
        # Step 1
        x, _ = top_cards[0]
        ...
        svg.text(
            x + 145,
            295,
            ["napari layer", "local file or store", "bundled sample"],
            cls="body",
            line_height=30,
        )
        svg.text(
            x + 150,
            406,
            "axes · calibration · source identity tracked",
            cls="small",
            anchor="middle",
        )
        ...
        svg.text(
            x + 150,
            390,
            ["add scientific operations", "connect compatible typed ports"],
            cls="body-sm",
            anchor="middle",
            line_height=27,
        )
        ...
        svg.raw(
            f'<rect x="{x + 186:g}" y="296" width="62" height="12" rx="6" '
            'fill="#c4b3e8"/>'
            f'<rect x="{x + 186:g}" y="322" width="82" height="12" rx="6" '
            'fill="#c4b3e8"/>'
            f'<rect x="{x + 186:g}" y="348" width="70" height="12" rx="6" '
            'fill="#c4b3e8"/>'
        )
        svg.text(
            x + 150,
            406,
            "previews · metadata · histograms · tables",
            cls="small",
            anchor="middle",
        )
        ...
        svg.text(
            x + 75,
            226,
            ["Review and save", "the authored state"],
            cls="card-title-sm",
            line_height=27,
        )
        ...
        svg.raw(
            f'<path d="M{x + 173:g} 335 l12 12 23 -29" fill="none" '
            'stroke="#a9680b" stroke-width="4"/>'
        )
        svg.text(
            x + 150,
            395,
    ...
    with svg.group("reuse-routes", label="Four reusable execution steps"):
        ...
        svg.text(
            x + 75,
            648,
            ["Reviewed workflow", "+ source bindings"],
            cls="card-title-sm",
            line_height=27,
        )
        svg.text(
            x + 150,
            725,
            ["graph · parameters", "portable compute intent"],
            cls="body",
            anchor="middle",
            line_height=30,
        )
        ...
        for y, label in routes:
            svg.rect(
                ...
                cls="mini-card",
                rx=10,
                extra='fill="#ffffff" stroke="#7a879c"',
            )
        ...
        svg.text(
            x + 75,
            648,
            ["Run the shared", "scientific executor"],
            cls="card-title-sm",
            line_height=27,
        )
        svg.rect(
        ...
        svg.text(
            x + 150,
            827,
            "unsupported segments visibly use CPU",
            cls="small",
            anchor="middle",
        )
        ...
        svg.text(
            ...
            733,
            [
                "images · masks · label images",
                "measurement tables",
                "execution report and provenance",
            ],
            cls="body-sm",
            anchor="middle",
            line_height=31,
    ...
    with svg.group("workflow-definition-note", label="Workflow persistence boundary"):
        ...
        svg.text(
            720,
            934,
            "Workflow JSON stores the authored graph, parameters, layout, and "
            "compute intent—not image arrays or runtime caches.",
            cls="body-sm",
            anchor="middle",
        )
...
def build_software_architecture() -> None:
    svg = SVG(
        ...
        "Interactive napari use, generated Python, and local collection batch "
        "execution converge on a shared Qt-free scientific core. Source payloads "
        "and normalized I/O feed the typed workflow graph and operation catalog. "
        "The isolated executor selects CPU reference kernels or eligible CUDA "
        "segments, and accepted results become napari layers or durable run "
        "records.",
    )
    ...
    with svg.group("title-band", label="Figure title"):
        ...
        svg.text(
            60,
            104,
            "Interactive and reusable routes converge on one metadata-aware "
            "workflow model and isolated scientific executor.",
            cls="subtitle",
        )
    ...
    with svg.group(
        "architecture-connectors", label="Architecture dependencies and execution flow"
    ):
        ...
        svg.path(
            "M720 780 H855 V802", cls="connector-green optional", marker="arrow-green"
        )
        svg.path("M1130 700 V802", cls="connector-amber", marker="arrow-amber")

    with svg.group(
        "interactive-entry", label="Interactive napari dock and application layer"
    ):
        ...
        svg.text(
            272, 332, "VIPP application layer", cls="card-title-sm", anchor="middle"
        )
        svg.text(
            272,
            349,
    ...
    with svg.group(
        "generated-code-entry", label="Generated Python and command line entry"
    ):
        ...
        svg.text(
            770,
            269,
            ["embedded workflow definition", "shared headless contract"],
            cls="body",
            anchor="middle",
            line_height=33,
        )
        svg.text(
            720,
            352,
            "workflow export does not require a completed run",
            cls="small",
            anchor="middle",
        )
    ...
    with svg.group("batch-entry", label="Local collection batch entry"):
        ...
        svg.rect(
            ...
            cls="mini-card",
            rx=12,
            extra='fill="#ffffff" stroke="#68758c"',
        )
        svg.rect(
            1205,
            252,
        ...
        svg.text(
            1165,
            338,
            ["deterministic plan · preflight", "fresh detached graph per item"],
            cls="body-sm",
            anchor="middle",
            line_height=28,
        )
    ...
    with svg.group("shared-core", label="Shared Qt-free scientific core"):
        ...
        svg.text(
            565,
            468,
            "scientific contracts throughout: axes · physical grid · source "
            "revision · actual backend record",
            cls="small-strong",
        )
        ...
        svg.text(
            310,
            552,
            ["Workflow graph", "& operation catalog"],
            cls="card-title",
            anchor="middle",
            line_height=29,
        )
        svg.text(
            ...
            620,
            [
                "typed DAG · ports · parameters",
                "ImageState · axis/grid contracts",
                "runtime cache owned by the graph",
            ],
            cls="body-sm",
            anchor="middle",
            line_height=27,
        ...
        svg.text(
            720,
            552,
            ["Execution planner", "& isolated runner"],
            cls="card-title",
            anchor="middle",
            line_height=29,
        )
        svg.text(
            ...
            620,
            [
                "dirty subgraph · manual barriers",
        ...
        svg.text(
            ...
            620,
            [
                "workflow JSON · generated Python",
                "plan · stage · reverify · promote",
    ...
    with svg.group("supporting-systems", label="I/O compute backends and outputs"):
        svg.text(
            225, 844, "Source payloads & I/O", cls="card-title-sm", anchor="middle"
        )
        svg.text(
            225,
            877,
            ["napari snapshots · TIFF · OME-Zarr", "NumPy · optional readers"],
            cls="body-sm",
            anchor="middle",
            line_height=27,
        )
        ...
        svg.text(
            555, 844, "CPU scientific kernels", cls="card-title-sm", anchor="middle"
        )
        svg.text(
            555,
            879,
            ["authoritative", "portable baseline"],
            cls="body-sm",
            anchor="middle",
            line_height=27,
        )
        ...
        svg.text(
            855,
            892,
            ["CuPy / CuPyX segments", "visible CPU fallback"],
            cls="body-sm",
            anchor="middle",
            line_height=24,
        )
        ...
        svg.text(
            1200, 844, "Results & run records", cls="card-title-sm", anchor="middle"
        )
        svg.text(
            1200,
            877,
            ["napari layers · images · tables", "workflows · manifests · provenance"],
            cls="body-sm",
            anchor="middle",
            line_height=27,
        )
...
def build_processing_pathways() -> None:
    svg = SVG(
        ...
        "Prepared multidimensional images can enter image restoration, "
        "segmentation and measurement, colocalization, or skeleton and network "
        "branches. Thresholding creates a mask, mask cleanup preserves the mask "
        "role, and connected components or watershed create label images. Visual "
        "quality-control outputs remain inspectable while table outputs merge "
    ...
    with svg.group("title-band", label="Figure title"):
        ...
        svg.text(
            60,
            104,
            "Typed VIPP graphs may omit, reorder, branch, or recombine compatible "
            "operations; the pathways below are representative.",
            cls="subtitle",
        )
    ...
    with svg.group("scientific-context", label="Scientific context"):
        ...
        svg.text(
            330,
            168,
            "semantic axes · physical scale and units · source identity · "
    ...
    with svg.group("prepared-image-hub", label="Prepared image inputs"):
        ...
        svg.text(
            ...
            735,
            [
                "Completed image-like outputs",
                "remain available for preview",
                "or full-resolution inspection.",
            ],
            cls="small",
            anchor="middle",
            line_height=24,
    ...
    with svg.group(
        "image-preparation-restoration", label="Image preparation and restoration lane"
    ):
        ...
        svg.text(
            470,
            344,
            ["channels · axes · regions", "data type · projections"],
            cls="small",
            anchor="middle",
            line_height=22,
        )
        ...
        svg.text(
            740,
            344,
            ["denoise · background", "detail and edge filters"],
            cls="small",
            anchor="middle",
            line_height=22,
        )
        ...
        svg.text(
            1040, 278, "measured/generated · validated", cls="small", anchor="middle"
        )

        svg.rect(920, 305, 240, 75, cls="card green optional", rx=15)
        svg.text(
            1040, 332, "RL / RL-TV restoration", cls="card-title-sm", anchor="middle"
        )
        svg.text(
            1040, 359, "2D or 3D · optional branch", cls="body-sm", anchor="middle"
        )
    ...
    with svg.group(
        ...
        label="Segmentation object separation and measurement lane",
    ):
        svg.text(
            350, 455, "SEGMENTATION, OBJECT SEPARATION & MEASUREMENT", cls="section"
        )
        ...
        svg.text(
            460,
            558,
            ["global or local threshold", "binary output"],
            cls="body-sm",
            anchor="middle",
            line_height=27,
        )
        ...
        svg.text(
            700,
            558,
            ["morphology · holes", "small objects · outliers"],
            cls="body-sm",
            anchor="middle",
            line_height=27,
        )
        ...
        svg.text(
            965, 525, "Separate / label objects", cls="card-title-sm", anchor="middle"
        )
        svg.text(
            965,
            558,
            ["connected components", "or watershed"],
            cls="body-sm",
            anchor="middle",
            line_height=27,
        )
        ...
        svg.text(
            1245,
            518,
            ["Object, intensity", "& mesh measurements"],
            cls="card-title-sm",
            anchor="middle",
            line_height=26,
        )
        svg.text(
            1245,
            580,
            ["physical quantities when", "valid calibration is available"],
            cls="small",
            anchor="middle",
            line_height=23,
        )
    ...
    with svg.group(
        "specialized-analysis", label="Colocalization and skeleton analysis lane"
    ):
        ...
        svg.text(
            590,
            780,
            "Colocalization & spatial association",
            cls="card-title-sm",
            anchor="middle",
        )
        svg.text(
            590,
            808,
            "two channels · optional ROI/labels · visual QC + metrics",
            cls="body-sm",
            anchor="middle",
        )
        ...
        svg.text(
            1110,
            808,
            "binary mask → skeletonize → branch/network QC",
            cls="body-sm",
            anchor="middle",
        )
    ...
    with svg.group(
        "table-composition-and-output", label="Table composition and scientific outputs"
    ):
        svg.rect(440, 930, 450, 105, cls="card amber", rx=18)
        svg.text(
            665, 970, "Compose analysis-ready tables", cls="card-title", anchor="middle"
        )
        svg.text(
            665,
            1004,
            "merge · select columns · add metadata · summarize",
            cls="body-sm",
            anchor="middle",
        )
        ...
        svg.text(
            1155, 970, "Inspect, save, or batch", cls="card-title", anchor="middle"
        )
        svg.text(
            1155,
            1004,
            "images · masks · label images · CSV / TSV tables",
            cls="body-sm",
            anchor="middle",
        )
...
def build_batch_provenance() -> None:
    svg = SVG(
        ...
        "Run-level collection binding, deterministic planning, and representative "
        "scientific preflight precede a per-item integrity loop. Each item "
        "captures source identity, runs a fresh workflow while recording actual "
        "implementations and cleanup evidence, stages available outputs "
        "privately, reverifies every input, and promotes valid artifacts one at a "
        "time. Integrity failures withhold new publication; execution errors may "
        "still yield a recorded partial item when completed outputs pass "
        "publication gates.",
    )
    ...
    with svg.group("title-band", label="Figure title"):
        ...
        svg.text(
            60,
            104,
            "Run-level planning is separated from per-item execution, staging, "
            "source reverification, and artifact promotion.",
            cls="subtitle",
        )
    ...
    with svg.group(
        "batch-connectors", label="Batch planning item execution and integrity outcomes"
    ):
    ...
    with svg.group("run-level-planning", label="Run-level setup steps"):
        ...
        svg.text(
            170,
            273,
            ["folder / glob bindings", "one or multiple workflow sources"],
            cls="body",
            line_height=28,
        )
        svg.text(
            255,
            329,
            ["source arrays remain external", "to workflow JSON"],
            cls="small",
            anchor="middle",
            line_height=20,
        )
        ...
        svg.text(
            720,
            320,
            ["sorted-position pairing · output paths", "collision policy"],
            cls="body-sm",
            anchor="middle",
            line_height=24,
        )
        ...
        svg.text(
            1070,
            226,
            ["Representative scientific", "preflight"],
            cls="card-title-sm",
            line_height=27,
        )
        svg.rect(
            ...
            cls="mini-card optional",
            rx=10,
            extra='fill="#ffffff" stroke="#7251b5"',
        )
        ...
        svg.text(
            ...
            289,
            [
                "Run repeats planning",
                "and checks a representative",
                "source set before items.",
            ],
            cls="body-sm",
            line_height=23,
        )
    ...
    with svg.group(
        "per-item-connectors", label="Per-item sequence and integrity outcomes"
    ):
        ...
        svg.path(
            "M532 645 V685 H300 V727", cls="connector-red soft-dash", marker="arrow-red"
        )
        svg.text(
            416,
            678,
            "cleanup unproven / early cancellation",
            cls="arrow-label",
            anchor="middle",
        )
        svg.path(
            "M1190 645 V705 H500 V727",
            cls="connector-red soft-dash",
            marker="arrow-red",
        )
        svg.text(
            845, 698, "source identity changed", cls="arrow-label", anchor="middle"
        )
    ...
    with svg.group("per-item-loop", label="Four per-item processing steps"):
        cards = [
            (
                70,
                1,
                "Capture identity & load",
                ("verify every input", "detach revision-safe payload"),
                "blue",
            ),
            (
                390,
                2,
                "Execute a fresh graph",
                ("record actual implementations", "establish cleanup evidence"),
                "green",
            ),
            (
                710,
                3,
                "Stage available outputs",
                ("private paths only", "completed branches may be partial"),
                "amber",
            ),
            (
                1030,
                4,
                "Reverify every source",
                ("compare captured identity", "decide whether to publish"),
                "neutral",
            ),
        ]
    ...
    with svg.group("integrity-outcomes", label="Withheld and published outcomes"):
        ...
        svg.text(
            1070, 838, "images · masks · labels · tables", cls="small", anchor="middle"
        )
        svg.text(
            1070,
            863,
            "Atomic per eligible artifact; later failure is recorded as a partial "
            "item.",
            cls="small",
            anchor="middle",
        )
        ...
        svg.text(
            265,
            920,
            "run manifest · checkpoints · actual backend · integrity decision · "
            "published / partial / withheld / failed / cancelled",
            cls="small",
        )
    ...
    with svg.group("next-item-loop", label="Return to the next planned item"):
        ...
        svg.text(
            660,
            958,
            "if policy continues and planned items remain",
            cls="arrow-label",
            anchor="middle",
        )
...
def build_processing_example() -> None:
    ...
    svg = SVG(
        ...
        "A single left-to-right process line shows a deterministic checked-in "
        "CZYX uint16 volume moving through the bundled Portable GPU Segmentation "
        "Bridge workflow. The third channel is converted to float32, Gaussian "
        "blurred, thresholded, cleaned in 3D, hole filled, and labeled into four "
        "connected components. The 3D cavity is demonstrated in orthogonal XY, "
        "XZ, and YZ index views.",
    )
    ...
    with svg.group("title-band", label="Figure title"):
        ...
        svg.text(
            60,
            104,
            "One deterministic synthetic CZYX volume moves through six supported "
            "VIPP operations in a single left-to-right path.",
            cls="subtitle",
        )
        svg.text(
            60,
            158,
            "ONE REVIEWED VOLUMETRIC PATH  ·  ALL CLEANUP AND LABELING USE THE "
            "COMPLETE ZYX VOLUME",
            cls="section",
        )
    ...
    with svg.group("worked-example-stages", label="Six real workflow stages"):
        ...
        svg.text(
            ...
            536,
            [
                "CZYX uint16 · 3 channels",
                "selected index 2 (third)",
                "12 × 96 × 128 ZYX",
            ],
            cls="body-sm",
            line_height=29,
        ...
        svg.text(
            372,
            536,
            ["float32", "scaling = Preserve", "Gaussian σ = 1.2"],
            cls="body-sm",
            line_height=29,
        )
        ...
        svg.text(
            ...
            [
                "18,180.2695",
                f"{evidence['threshold_foreground_voxels']:,} foreground voxels",
        ...
        svg.text(
            ...
            [
                "minimum 22 voxels",
                f"{evidence['removed_speck_voxels']}-voxel speck removed",
                f"{evidence['after_remove_small_objects_voxels']:,} mask voxels remain",
            ],
            cls="body-sm",
            line_height=29,
        )
        ...
        svg.rect(
            ...
            cls="mini-card",
            rx=10,
            extra='fill="#ffffff" stroke="#8d9ab0"',
        )
        svg.image(
            1298,
            287,
        ...
        svg.text(
            ...
            [
                ...
                "1 + 29 + 1 across z = 2–4",
                f"{evidence['after_remove_small_objects_voxels']:,} → "
                f"{evidence['after_fill_holes_voxels']:,} mask voxels",
            ],
            cls="body-sm",
            line_height=26,
        )
    ...
    with svg.group(
        "example-evidence-notes", label="Display and execution qualifications"
    ):
        svg.rect(60, 800, 1780, 45, cls="band band-neutral", rx=14)
        svg.text(
            950,
            829,
            "Intensity projections are independently normalized for display; "
            "Stage 5 shows real XY/XZ/YZ index views. All operations and counts "
            "use the complete ZYX volume.",
            cls="small-strong",
            anchor="middle",
        )
        ...
        svg.text(
            950,
            885,
            "The checked-in panels use authoritative CPU results. Prefer GPU "
            "records intent; actual CPU/GPU decisions and visible fallback remain "
            "explicit.",
            cls="small",
            anchor="middle",
        )
