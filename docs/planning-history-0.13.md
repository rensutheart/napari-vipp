# Archived napari-vipp Planning Through 0.13.0a7

> **Status:** Historical snapshot archived on 2026-08-20. It preserves the
> release chronology, engineering rationale, and qualification detail that had
> accumulated in the active roadmap. It is not the current source of release
> order. See the [active roadmap](planning.md), the
> [product ideas](product-ideas.md), and the [changelog](../CHANGELOG.md).

Original snapshot review date: 2026-08-14

The text below is preserved as the roadmap that was active on that date. Its
references to "current," "next," or active priority describe the historical
0.13 planning state, not present ownership. Detailed delivered facts remain
canonical in the [changelog](../CHANGELOG.md) and specialist records.
...
Current published alpha release: `0.13.0a7`.

The `0.13.0a7` public baseline includes:
...
Iterative alphas use the risk-based
[release runbook](release-runbook.md): exact artifact facts are regenerated,
while unchanged installer, GPU, schema, and UI evidence carries forward from
the [qualification baseline](release-qualification-baseline.md). Release
candidates and stable production milestones apply broader maturity gates.
...
class, downloaded-installer fresh-account acceptance, and the wider
Windows/Linux qualification targets remain open; the public support matrix
must not expand until those results exist.
